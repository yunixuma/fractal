...
/*   Updated: 2024/02/26 18:29:10 by Yoshihiro K      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (pixel.x < WIDTH_VISION)
	{
		...
		while (pixel.y < HEIGHT_VISION)
		{
			...
			if (var->param->type == TYPE_MANDELBROT)
				iter = fra_calc_mandelbrot(coord, var->param->constant, \
					var->param->cycle);
			else if (var->param->type == TYPE_JULIA)
				iter = fra_calc_julia(coord, var->param->constant, \
					var->param->cycle);
			else if (var->param->type == TYPE_BURNINGSHIP)
				iter = fra_calc_burningship(coord, var->param->constant, \
					var->param->cycle);
			else
...
// DD(coord.y);
				fra_render_pixel(&var->img[IDX_VISION], \
					var->param->color[iter % N_COLOR], &pixel);
