...
/*   Updated: 2024/02/21 01:23:47 by Yoshihiro K      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*	t_vec	ray;
	...
	{
		angle = mlx->game.angle.yaw + ft_math_rad2deg(atan(tan(ft_math_deg2rad(mlx->game.zoom / 2)) * i_col / (WIDTH_VISION / 2)));
...
/*
	tan(a) * k = i_col
tan(zoom/2) * k = WIDTH_VISION >> 1
tan(a) / tan(zoom/2) = i_col / WIDTH_VISION / 2
a = atan(tan(zoom/2) * i_col / WIDTH_VISION / 2)
*/

//		angle = mlx->game.angle.yaw + mlx->game.zoom * i_col / WIDTH_VISION;
//		angle = mlx->game.angle.yaw + ft_math_rad2deg(asin(ft_math_sin_deg(mlx->game.zoom / 2) * i_col * 2 / WIDTH_VISION));

/*
k sin(a) = i_col
k sin(zoom/2) = WIDTH_VISION/2
i_col / sin(a) = WIDTH_VISION/2 / sin(zoom/2)
sin(a) = sin(zoom/2) * i_col * 2/WIDTH_VISION
a = asin(sin(zoom/2) * i_col * 2/WIDTH_VISION)
*/
/*
void	c3d_render_visible(t_mlx *mlx)
{
	...
	while (i_col < WIDTH_VISION / 2)
	{
		pt.x = mlx->game.coord.x + (mlx->game.coord.y - pt.y) \
			* tan(ft_math_deg2rad(mlx->game.zoom * i_col / WIDTH_VISION));
